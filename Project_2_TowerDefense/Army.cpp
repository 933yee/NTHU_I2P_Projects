...
Army::Army(std::string img, float x, float y, float radius, float coolDown, float speed, float hp, int id, float shootRadius, float w, float h) :
    Role(img, x, y, w, h), coolDown(coolDown), speed(speed), id(id), shootRadius(shootRadius) {
    CollisionRadius = radius;
    HP = hp;
    reachEndTime = 0;
    wallPos = Engine::Point(-1, -1);
