...
void PlayScene::Initialize() {
    ...
    front = nullptr;
    back = nullptr;
    ArmyImage[2] = "play/Pika-2.png";
    ArmyImage[6] = "play/ice.png";
    armyAmount[6] = 2;
	mapState.clear();
	keyStrokes.clear();
	ticks = 0;
    ...
    for (int i = 0; i < 9; i++) {
        imgIce[i] = new Engine::Image("play/frozen7.png", 0, 0, BlockSize, BlockSize);
        imgIce[i]->Visible = false;
        UIGroup->AddNewObject(imgIce[i]);
    }
...
void PlayScene::Update(float deltaTime) {
    if (totalTime == 120) {
        AddNewObject(front = new Engine::Label("Time: ", "ARCADE.ttf", 50, 90, 45, 0, 0, 0, 255, 0.5, 0.5));
        AddNewObject(timeLeft = new Engine::Label("120.0 ", "ARCADE.ttf", 50, 190, 45, 255, 0, 0, 255, 0.5, 0.5));
        AddNewObject(back = new Engine::Label(" s", "ARCADE.ttf", 50, 255, 45, 0, 0, 0, 255, 0.5, 0.5));
    }
	if (SpeedMult == 0)
		deathCountDown = -1;
    ...
    if(flag) Engine::GameEngine::GetInstance().ChangeScene("win");
    ...
    totalTime -= deltaTime * SpeedMult;
    std::string tmp = std::to_string(totalTime);
    timeLeft->Text = tmp.substr(0, tmp.find(".") +2);
    if (totalTime <= 0 || armyEmpty) {
        preview = nullptr;
        ...
        delete EffectGroup;
        delete UIGroup;
        RemoveObject(timeLeft->GetObjectIterator());
        RemoveObject(front->GetObjectIterator());
        RemoveObject(back->GetObjectIterator());
        objects.clear();
        controls.clear();
...
void PlayScene::OnMouseMove(int mx, int my) {
    const int dir[9][2] = { { BlockSize, 0}, {0,  BlockSize}, { BlockSize,  BlockSize}, {-BlockSize, 0}, {0, -BlockSize}, {-BlockSize, -BlockSize}, {0, 0}, { BlockSize, -BlockSize}, {-BlockSize,  BlockSize} };
    ...
    if (preview->id == 6) {
        for (int i = 0; i < 9; i++) {
            imgIce[i]->Visible = true;
            imgIce[i]->Position.x = x * BlockSize + dir[i][0];
            imgIce[i]->Position.y = y * BlockSize + dir[i][1];
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if (button & 1) {
		if (!CheckOccupied(x, y) || preview->id == 6) {
			if (!preview)
				return;
            if(preview->id == 2) AudioHelper::PlayAudio("PikachuSound.wav");
            ...
            if (remainId != -1) {
                ...
                if (remainId == 2)
                    preview = new PikaArmy(0, 0);
                if (remainId == 3)
                    preview = new sion(0, 0);
                if (remainId == 4)
                    preview = new doge_tank(0, 0);
                if (remainId == 5)
                    preview = new priest(0, 0);
                if (remainId == 6)
                    preview = new IceArmy(0, 0);
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        PlayScene::DebugMode = PlayScene::DebugMode ? false : true;
        for (auto& it : DefenseGroup->GetObjects()) {
            Defense* defense = dynamic_cast<Defense*>(it);
            if (defense->id == 7) {
                defense->debug = defense->debug? false : true;
                if (defense->debug) {
                    defense->Tint = al_map_rgba(255, 255, 255, 150);
                }
                else defense->Tint = al_map_rgba(255, 255, 255, 0);
            }   
        }
	}
	else {
        ...
        if (keyStrokes.size() == 7) {
            ...
            if (flag) EffectGroup->AddNewObject(new Plane());
        }
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_FAT_CANNON); break;
        case '4': mapData.push_back(MULTI_TURRENT_CANNON); break;
        case '5': mapData.push_back(WIZARD); break;
        case '7': mapData.push_back(MINES); break;
		case '\n':
		case '\r':
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_FAT_CANNON:
                    DefenseGroup->AddNewObject(new fat_cannon_Defense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    
                    break;
                case MULTI_TURRENT_CANNON:
                    DefenseGroup->AddNewObject(new multi_turrent_Defense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                  
                    break;
                case MINES:
                    DefenseGroup->AddNewObject(new mines_Defense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                 
                    break;
                case WIZARD:
                    DefenseGroup->AddNewObject(new wizard(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
...
bool PlayScene::CheckOccupied(int x, int y) {
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt == TILE_FAT_CANNON || tt == MULTI_TURRENT_CANNON || tt == WIZARD) return true;
